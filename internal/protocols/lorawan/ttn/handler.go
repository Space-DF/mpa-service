...
// Handler implements HTTP handler for TTN webhooks
type Handler struct {
...
// NewHandler creates a new TTN handler
...
// Handle processes TTN webhook requests
func (h *Handler) Handle(c echo.Context) error {
	...
	if err != nil {
		h.logger.Errorf("TTN: Error reading request body: %v", err)
		return echo.NewHTTPError(400, "Failed to read request body")
	}
	
	h.logger.Infof("TTN: Received request from %s (body: %d bytes)", 
		c.Request().RemoteAddr, len(body))
	
	// Add TTN source to metadata
	...
	if err := h.deviceService.ProcessHTTPMessage(c.Request(), body, transportMetadata); err != nil {
		h.logger.Errorf("TTN: Error processing message: %v", err)
		return echo.NewHTTPError(500, "Failed to process message")
	}
	
	h.logger.Infof("TTN: Successfully processed message in %v", time.Since(startTime))
